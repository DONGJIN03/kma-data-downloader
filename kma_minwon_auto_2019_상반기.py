# -*- coding: utf-8 -*-
"""
기상청 전자민원 - 기상특보(해상) [2019년 상반기 보완용]
동해남부/서해남부 먼바다 x 옛 구역명(~19.04.30) x 기간 1.1~4.30 만 신청 (4건)
* 개편일 2019.4.30. 이 두 해역 외 먼바다/앞바다는 pre2021로 1년치 이미 수집 전제
* 개편 이후(5.1~)는 ~21.07.29 구역명으로 pre2021이 담고 있으므로, 그 파일엔 _0501-1231 꼬리표를 rename으로 붙일 것
==============================================================
사전 준비:
...
ZONE_GROUPS = {
    # 2019.1.1~4.30 보완: 4.30 개편으로 북/남 나뉘기 전의 통짜 옛 구역명
    # (동해남부·서해남부 먼바다만 해당. 나머지 해역/앞바다는 pre2021로 수집 완료)
...
def run_apply(page, year, wcode, zones, dry):
    ...
    page.evaluate(
        """(v) => {
            ...
            return 'ok';
        }""", {"start": f"{year}0101", "end": f"{year}0430"})   # 상반기(4.30 개편 전)만
    ...
    for z in zones:
        zb = z.split("(~")[0]          # 옛 구역명은 날짜 꼬리표 떼고 대조
...
def fetch_new_pdf(page, prev_no, year, sea, part, wcode):
    ...
    fname = f"기상특보(해상)_{year}_{sea}_{part}_{WARN_FULL[wcode]}_0101-0430.pdf"
