# -*- coding: utf-8 -*-
"""
기상청 전자민원 - 기상특보(해상) [2021년 상반기 보완용]
먼바다 x 옛 구역명 x 기간 1.1~7.29 만 신청 (12건)
앞바다 전체와 먼바다 7.30 이후는 기본 스크립트로 이미 수집 완료 전제
==============================================================
사전 준비:
...
ZONE_GROUPS = {
    # 2021.1.1~7.29 먼바다 보완: 옛 구역명만 체크 (앞바다/하반기 먼바다는 수집 완료)
    # 남해서부 먼바다는 '14년 개편이라 현행명으로 1년치가 이미 수집됨 -> 제외
...
def run_apply(page, year, wcode, zones, dry):
    ...
    page.evaluate(
        """(v) => {
            ...
            return 'ok';
        }""", {"start": f"{year}0101", "end": f"{year}0729"})   # 상반기(개편 전)만
...
def fetch_new_pdf(page, prev_no, year, sea, part, wcode):
    ...
    fname = f"기상특보(해상)_{year}_{WARN_FULL[wcode]}_0101-0729.pdf"
