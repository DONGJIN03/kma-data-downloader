# -*- coding: utf-8 -*-
"""
기상청 전자민원 - 기상특보(해상) 신청 + PDF 자동 다운로드 [개편연도형]
==============================================================
■ 이 파일은 "개편일이 한 해에 걸치는 연도"의 먼바다 반기 보완 전용이다.
  해당 연도: 2021 / 2019 / 2014
  한 해의 먼바다 자료가 개편일을 경계로 두 조각(옛명 구간 + 현행명 구간)으로
  나뉘므로, 이 파일로 "옛명 + 개편 전 기간"을 신청해 앞 조각을 받는다.
  (뒤 조각 = 개편 후 기간은 기본형 kma_minwon_auto.py로 이미 수집되는 전제)

■ 연도를 바꾸려면: 아래 설정에서 4개만 교체한다 (README.md에 연도별 블록 있음).
    1) YEARS         = [해당 연도]
    2) PERIOD_END    = "MMDD"   개편일. 2021:"0729" / 2019:"0430" / 2014:"1210"
    3) FILE_TAG      = "_0101-MMDD"  파일명 꼬리표(하반기와 안 겹치게)
    4) ZONE_GROUPS   = 그 개편으로 이름 바뀐 먼바다만, 개편 직전 옛 구역명으로

■ 기간 시작은 항상 그 해 0101. 끝만 PERIOD_END로 바뀐다.
  개편과 무관한 해역·앞바다는 넣지 않는다(기본형으로 이미 수집됨).
==============================================================
사전 준비:
...
YEARS = [2021]                   # 개편 걸친 연도 하나 (2021/2019/2014)

# ── 개편연도 전용 설정 (연도마다 이 3개를 README 블록에서 복사해 교체) ──
PERIOD_END = "0729"              # 개편일 MMDD. 2021:"0729" 2019:"0430" 2014:"1210"
FILE_TAG   = "_0101-0729"        # 파일명 꼬리표. 2019:"_0101-0430" 2014:"_0101-1210"
DELAY_SEC = 4                    # 신청 간 대기
LOG_FILE = "신청로그.csv"
...
ZONE_GROUPS = {
    # ↓↓↓ 아래는 2021년 예시. 다른 연도는 README.md의 블록으로 통째 교체 ↓↓↓
    # 2021(개편일 7.29): 남해서부 제외 6개 해역 먼바다, 개편 직전 옛명(~'21.07.29)
    ("동해중부", "먼바다"): ["동해중부먼바다(~'21.07.29)"],
    ("동해남부", "먼바다"): ["동해남부북쪽먼바다(~'21.07.29)",
                          "동해남부남쪽먼바다(~'21.07.29)"],
    ("서해중부", "먼바다"): ["서해중부먼바다(~'21.07.29)"],
    ("서해남부", "먼바다"): ["서해남부북쪽먼바다(~'21.07.29)",
                          "서해남부남쪽먼바다(~'21.07.29)"],
...
def run_apply(page, year, wcode, zones, dry):
    ...
    page.evaluate(
        """(v) => {
            ...
            return 'ok';
        }""", {"start": f"{year}0101", "end": f"{year}{PERIOD_END}"})   # 개편 전 기간만
...
def fetch_new_pdf(page, prev_no, year, sea, part, wcode):
    ...
    fname = f"기상특보(해상)_{year}_{sea}_{part}_{WARN_FULL[wcode]}{FILE_TAG}.pdf"
